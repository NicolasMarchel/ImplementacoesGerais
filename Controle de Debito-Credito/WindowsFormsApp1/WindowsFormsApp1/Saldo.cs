...
{
    class Saldo:IExecute
    {

        public void ExcutaOpeçao(Caixa caixa, ArrayList Cad, Registro registro)
        {
            double totalCredito, totalDebito;
            Debito totalD = new Debito();
            Credito totalC = new Credito();
            ...
            if (Cad.Count > 0)
            {
                if (caixa.checkMes.Checked)
                {
                    ...
                    totalDebito = totalD.Totalizaçao(Temp);
                    Temp.Clear();
                    for (int i = 0; i < Cad.Count; i++)
                    {
                        if (((Movimentaçao)Cad[i]).Data.Month == mes && (Cad[i]).GetType() == typeof(Credito))
                        {
                            Temp.Add(Cad[i]);
                        }
                    }
                    caixa.txtTotal.Visible = true;
                    totalCredito = totalC.Totalizaçao(Temp);

                    caixa.lblMensagem.Visible = true;
                    caixa.lblMensagem.Text = "O saldo em " + textoMes + " é...";
                    caixa.txtTotal.Enabled = true;
                    caixa.txtTotal.Text = "R$ " + (totalDebito - totalCredito).ToString();
                                ...
                                if (caixa.checkPeriodo.Checked)
                {
                    ...
                    for (int i = 0; i < Cad.Count; i++)
                    {
                        if (Cad[i].GetType() == typeof(Credito))
                        {
                    ...
                    caixa.txtTotal.Visible = true;
                    caixa.lblMensagem.Visible = true;
                    caixa.lblMensagem.Text = "O Saldo entre " + caixa.dtpInicial.Value.ToString().Substring(0, 10) +
